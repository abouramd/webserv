            ...
            if ((entry = readdir(client.dir)))
            {
                if (entry->d_name[0] != '.')
                {
                  ...
                  if( S_ISDIR(filestat.st_mode) )
                    dir = "<li><a href='./"  + std::string(entry->d_name) + "'><i class='bi bi-folder-fill'></i> " + std::string(entry->d_name) + "</a></i></li>";
                  else
                    dir = "<li><a href='./"  + std::string(entry->d_name) + "'><i class='bi bi-file-earmark-text-fill'></i> " + std::string(entry->d_name) + "</a></i></li>";
